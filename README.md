...
<div align="center">
  ...
  ![Groq](https://img.shields.io/badge/Groq-F55036?style=for-the-badge&logo=data:image/svg%2bxml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAyNCAyNCI+PC9zdmc+&logoColor=white)
  ![Render](https://img.shields.io/badge/Deployed_on-Render-46E3B7?style=for-the-badge&logo=render&logoColor=white)
  ![Responsive Design](https://img.shields.io/badge/Responsive-Mobile_to_4K-4CAF50?style=for-the-badge)
  ![License: MIT](https://img.shields.io/badge/License-MIT-green.svg?style=for-the-badge)
  ![Stars](https://img.shields.io/github/stars/KajalAhir23/Krishi-Cure-AI?style=for-the-badge&color=gold)
  ![Forks](https://img.shields.io/github/forks/KajalAhir23/Krishi-Cure-AI?style=for-the-badge&color=blue)
...
**🔗 Live App:** [krishi-cure-ai.onrender.com](https://krishi-cure-ai.onrender.com)
...
| **🔬 Hybrid Diagnosis Engine** | Combines image visual inspection with user-selected symptoms for higher prediction accuracy, backed by a dual-provider AI pipeline (Groq primary, Gemini fallback). |
...
The platform supports **75+ crops** across 3 primary categories (Field Crops, Vegetables, Fruits) — including major Indian staple, cash, and horticultural crops:
...
│ Wheat           │     │ Tomato          │     │ Banana          │
│ Cotton          │     │ Potato          │     │ Mango           │
│ Rice (Paddy)    │     │ Onion           │     │ Citrus / Orange │
│ Maize           │     │ Chilli          │     │ Papaya          │
│ Sugarcane       │     │ Brinjal         │     │ Guava           │
│ Groundnut       │     │ Cabbage         │     │ Pomegranate     │
│ Bajra / Jowar   │     │ Cauliflower     │     │ Grapes          │
│ + 25 more...    │     │ + 30 more...    │     │ + 15 more...    │
└─────────────────┘     └─────────────────┘     └─────────────────┘
```

> Full crop list with regional-language names is stored in `data/crops.json`.
...
- **Interactive Manual Location Picker**: Built-in map and search for selecting custom villages, districts, or cities.
...
- Powered by **OpenWeatherMap** when configured, with automatic fallback to the free **Open-Meteo** API if no key is provided — the weather module works out of the box with zero setup.
...
| **Styling & System** | Custom Vanilla CSS (Design Tokens) | Earthy premium design system, fluid clamp units, fully responsive 320px–4K. |
| **Backend Runtime** | Node.js (v16+) & Express.js (v5) | REST API routes, request sanitization, rate limiting. |
| **Artificial Intelligence**| Groq (`groq-sdk`) + Google Gemini (`@google/genai`) | Dual-provider AI pipeline — Groq as primary LLM with automatic failover to Gemini for both text and multimodal image diagnosis. |
| **Authentication** | Firebase Auth SDK | One-tap Google OAuth 2.0 single sign-on. |
| **Weather** | OpenWeatherMap API (optional) / Open-Meteo (free fallback) | Live weather data with zero-config fallback. |
| **Security & Rate Limit**| Express-Rate-Limit & CORS | API abuse prevention and secure cross-origin resource sharing. |
| **Deployment** | Render | Live production hosting with environment-based configuration. |
...
│   │   ├── premium-theme.css  # Earthy premium visual theme overlay
...
│   │   └── weather.js         # Weather API fetcher, map, & advisory engine
...
├── scratch/                   # Dev/test utilities, incl. accuracy benchmark scripts
│   ├── benchmark.js            # Image-based diagnosis accuracy benchmark
│   └── symptom-benchmark.js    # Symptom-based diagnosis accuracy benchmark
...
- A [Groq API key](https://console.groq.com) (primary AI provider)
- A [Google Gemini API key](https://aistudio.google.com/apikey) (fallback AI provider)
- A [Firebase](https://console.firebase.google.com) project (for Google Sign-In)
- *(Optional)* An [OpenWeatherMap API key](https://openweathermap.org/api) — weather works without one via the free Open-Meteo fallback
...
# Fallback AI Provider (required)
GEMINI_API_KEY=your_google_gemini_api_key_here

# Weather (optional — falls back to free Open-Meteo if omitted)
...
> [!WARNING]
> Never commit your actual `.env` file or private API keys to public version control repositories. On Render, set these as Environment Variables in the dashboard instead.
...
> - All crop disease predictions generated by **Krishi Cure AI** are AI-assisted evaluations based on multimodal visual analysis of user-uploaded images and manually selected plant symptoms — powered by large language models (Groq/Gemini), not a lab-certified diagnostic classifier.
> - The **Confidence Score** indicates the model's own estimated certainty in its prediction, not a statistically validated clinical accuracy rate.
> - If the confidence score is low or the disease symptoms appear severe and unrecoverable, users are strongly advised to consult a certified local agricultural extension officer, ICAR expert, or qualified agronomist before purchasing or applying intensive chemical treatments.
> - This application is an educational and advisory tool and does not claim 100% diagnostic accuracy under all environmental conditions. See the [AI Accuracy Benchmark](#-ai-accuracy-benchmark) section below for measured results.

---

## 🧪 AI Accuracy Benchmark

To validate diagnostic reliability rather than just claim it, two independent, automated benchmark scripts were built and run against the live diagnosis engine.

### Symptom-Based Diagnosis Benchmark
100 test cases auto-generated directly from the app's own symptom taxonomy (`data/crops.json`) across **40 crops** — no external dataset required. Each test checks whether the AI correctly classifies the underlying disease category (fungal / bacterial / viral / nutrient deficiency) from a realistic symptom combination.

| Metric | Result |
| :--- | :--- |
| Raw automated accuracy | 84.0% (84/100) |
| **Corrected accuracy** (after manual review of scoring-script bugs) | **92.0%** |
| Corrected accuracy (incl. genuinely ambiguous cases*) | 97.0% |
| Bacterial classification | 92.9% |
| Viral classification | 91.7% |
| Fungal classification | 82.5% |
| Nutrient deficiency classification | 79.4% |

<sub>*A manual review of every mismatch found most were false negatives in the automated scoring script (e.g. missed keyword synonyms), not model errors. A few cases were genuinely ambiguous even by real plant-pathology standards — e.g. Citrus Greening (HLB) is famously misdiagnosed as nutrient deficiency in real orchards because the symptoms are visually identical, and the AI's diagnosis in that case reflected accurate domain knowledge rather than error.</sub>

This benchmark also incidentally verified the app's **multi-provider AI failover** (Groq → Gemini) under real conditions — when Groq's rate limit was hit mid-run, the app automatically and successfully fell back to Gemini with no downtime.

### Image-Based Diagnosis Benchmark
A second benchmark tests the multimodal image diagnosis path against labeled leaf-disease datasets (PlantVillage, Top Agriculture Crop Disease India) covering Potato, Tomato, Capsicum, Rice, Wheat, and other supported crops.

> Full benchmark scripts are available in `scratch/benchmark.js` (image-based) and `scratch/symptom-benchmark.js` (symptom-based) for independent reproduction. Run with `node scratch/symptom-benchmark.js --cropsLimit=40`.
...
- 📶 **Offline Support (PWA)**: Progressive Web App functionality for accessing saved treatment guides in remote areas with low connectivity.
...
- 🧠 **Trained Image Classifier**: Add a fine-tuned CNN as a first-pass filter ahead of the LLM pipeline for higher-confidence image diagnoses.
...
- [Groq](https://groq.com/) — Primary high-speed LLM inference engine.
- [Google Gemini AI](https://deepmind.google/technologies/gemini/) — Multimodal AI analysis & conversation fallback.
- [OpenWeatherMap](https://openweathermap.org/) & [Open-Meteo](https://open-meteo.com/) — Real-time weather data & forecasts.
...
<div align="center">
  ...
  *B.Tech in Computer Science Engineering (Artificial Intelligence)*
  ...
  <p>
    <a href="https://github.com/KajalAhir23"><img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/></a>
    <!-- Add your LinkedIn: <a href="https://linkedin.com/in/your-real-handle"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/></a> -->
    <a href="mailto:kajalbhatiya610@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/></a>
  </p>
...
- **GitHub Repository**: [https://github.com/KajalAhir23/Krishi-Cure-AI](https://github.com/KajalAhir23/Krishi-Cure-AI)
- **Live Application**: [https://krishi-cure-ai.onrender.com](https://krishi-cure-ai.onrender.com)
